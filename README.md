...
`jaffle_lake` is a fictional ecommerce store. This dbt project transforms raw
...
![Jaffle Shop ERD](/etc/jaffle_lake_erd.png)

This [dbt](https://www.getdbt.com/) project has a split personality:
* **Tutorial**: The [tutorial](https://github.com/fishtown-analytics/jaffle_lake/tree/master)
...
* **Demo**: The [demo](https://github.com/fishtown-analytics/jaffle_lake/tree/demo/master)
...
3. Change into the `jaffle_lake` directory from the command line:
```bash
$ cd jaffle_lake
```

4. Set up a profile called `jaffle_lake` to connect to a data warehouse by
